...
while cap.isOpened():
    ...
    if len(lmList) != 0:
        ...
        r_ankle = detector.findAngle(img, 26,28,32)
        r_hip = detector.findAngle(img, 12, 24,26)
        
        #Percentage of success of lunges
        per = np.interp(r_knee, (90, 160), (0, 100))
        
        #Bar to show lunges progress
        bar = np.interp(r_knee, (90, 160), (380, 50))

        #Check to ensure right form before starting the program
        if r_knee > 160 and r_ankle > 40 and r_hip > 160:
            form = 1
    
        #Check for full range of motion for the lunges
        if form == 1:
            if per == 0:
                if r_knee <= 100 and r_hip > 160:
                    feedback = "Up"
                    if direction == 0:
                        count += 0.5
                        direction = 1
